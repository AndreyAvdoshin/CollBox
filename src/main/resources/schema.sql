...
-- Подумать над тем, чтобы изменить поле типа категории на ENUM
...
-- Подумать над тем, чтобы добавить в таблицу поля описания счета, а также даты открытия и активности счета, быть может поля времени изменения
...
CREATE OR REPLACE TRIGGER update_transfers_timestamp BEFORE UPDATE ON transfers
FOR EACH ROW
EXECUTE PROCEDURE set_updated();

CREATE OR REPLACE TRIGGER update_transactions_timestamp BEFORE UPDATE ON transactions
FOR EACH ROW
EXECUTE PROCEDURE set_updated();